...
4. ```sql
    ...
    or the duckbd way:
    ```sql
    describe from invoices;
    ```
5. Let's create a view of average check per coutry per day.

    ```sql
    create view v_average_checks as select BillingCountry, date_trunc('day', InvoiceDate) as day, avg(Total) as avg_check from invoices group by BillingCountry, day;
    ```
    
    from invoices limit 2;
    ```
